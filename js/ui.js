...
function setup(game) {
  ...
  <main class="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-10">
    ...
    <div class="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
      ...
      <section class="${card} p-5 sm:p-6"><div class="mb-5 flex items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-white">2. Composition</h2><p class="text-sm text-stone-400">Suggestion modifiable librement</p></div>${btn("Composition recommandée", "recommend-roles", "secondary")}</div>
        ...
        <div class="mt-5 rounded-xl border ${rolesCount === count ? "border-moss/30 bg-moss/10 text-[#bbd2b2]" : "border-amberwood/30 bg-amberwood/10 text-amber-200"} p-3 text-sm">${rolesCount === count ? "La composition est prête." : `Écart : ${Math.abs(count - rolesCount)} rôle${Math.abs(count - rolesCount) > 1 ? "s" : ""}. Ajustez la composition.`}</div>
